...
class BanditLearner():
    ...
    def __init__(self, actions, epsilon):
        ...
        self.estimates = {act: 0 for act in actions}
        self.action_counts = {act: 0 for act in actions}

    def stepsize(self, action, t):
        return 1 / (self.action_counts[action])

    def select_best(self, t):
...
class UCBBandit(BanditLearner):
    ...
    def __init__(self, actions, epsilon, confidence):
    ...
    def select_best(self, t):
        def upper_bound(act):
            ...
            uncertainty = self.c * np.sqrt(np.log(t + 1) / (self.action_counts[act] + 1e-8))
